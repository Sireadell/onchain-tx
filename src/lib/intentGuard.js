...
const ACADEMIC_EXCLUDE = String.raw`(?:news|newspaper|shopping|product\s+review|police|personal\s+journal|cheapest\s+flights?|flight\s+prices?)`;
const ACADEMIC_EXCLUDE_RE = new RegExp(String.raw`\b${ACADEMIC_EXCLUDE}\b`, 'i');
const ACADEMIC_WORD_CUES = /\b(?:academic|papers?|peer[- ]reviewed|scholars?|scholarly|cit(?:e|ed|ation|ations)|dissertation|meta[- ]analysis|research\s+stud(?:y|ies)|stud(?:y|ies)\s+(?:paper|papers|research)|journal\s+(?:paper|papers|article|articles)|scientific\s+articles?)\b/i;
const ACADEMIC_IMPERATIVE = /^(?:find|show(?:\s+me)?|get|search\s+for)\b.*\b(?:literature|publications?|articles?)\b/i;
const ACADEMIC_LITERATURE_SAYS = /\bliterature\s+(?:say|says)\b/i;
const ACADEMIC_RESEARCH_IMPERATIVE = /^(?:please\s+)?research\s+\S.+/i;
