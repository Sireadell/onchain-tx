...
const ANKR_CALL_TIMEOUT_MS = Number(process.env.ANKR_CALL_TIMEOUT_MS) || 4_000;
...
const MAX_ANALYSIS_TIME_MS = Number(process.env.MAX_ANALYSIS_TIME_MS) || 12_000;
...
const RATE_LIMIT_BURST = Number(process.env.ANKR_RATE_LIMIT_BURST) || 20;
...
async function fetchAnkrRpc(chainSegment, method, params) {
  ...
  for (let attempt = 0; attempt <= RETRY_DELAYS_MS.length; attempt++) {
    ...
    const budget = budgetContext.getStore();
    const remainingBudgetMs = budget ? Math.max(1, budget.deadlineAt - Date.now()) : ANKR_CALL_TIMEOUT_MS;
    const callTimeoutMs = Math.min(ANKR_CALL_TIMEOUT_MS, remainingBudgetMs);
    ...
    if (!retryable || !attemptsLeft) {
      if (errName === 'AbortError') {
        if (budget && Date.now() >= budget.deadlineAt) {
          throw new RpcBudgetExceededError('max analysis time exceeded');
        }
        throw new Error(`Ankr request timed out after ${callTimeoutMs}ms (${attempt + 1} attempt(s))`);
      }
    ...
    if (budget && Date.now() + jitteredDelay >= budget.deadlineAt) {
      throw new RpcBudgetExceededError('max analysis time exceeded');
    }
